...
export const MongoDbLayer: DBLayer = {
    ...
    getObject: async (pCollection: string, pCriteria: CriteriaFilter, pCollation?: any) => {
        if (pCollation) {
    ...
    async *getObjects(pCollection: string, pPager?: CriteriaFilter, pInfoer?: CriteriaFilter, pScoper?: CriteriaFilter): AsyncGenerator<any> {
    ...
    updateObjectFields: async (pCollection: string, pCriteria: CriteriaFilter, pFields: VKeyedCollection) => {
        let doSet = false;
        let doUnset = false;
    ...
    countObjects: async (pCollection: string, pCriteria: CriteriaFilter) => {
        Logger.cdebug('db-query-detail', `Db.countObjects: collection=${pCollection}, criteria=${JSON.stringify(pCriteria.criteriaParameters())}`);
        return Datab.collection(pCollection).countDocuments(pCriteria.criteriaParameters() as FilterQuery<any>);
    },
};
