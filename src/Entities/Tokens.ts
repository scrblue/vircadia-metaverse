...
export function initTokens(pDB: DBLayer): void {
    dbLayer = pDB;
    // Expire tokens that have pased their prime
    setInterval( async () => {
        const nowtime = new Date();
        const deletedTokens = await dbLayer.deleteMany(tokenCollection,
...
export const Tokens = {
    ...
    async getTokenWithTokenId(pTokenId: string): Promise<AuthToken> {
        return IsNullOrEmpty(pTokenId) ? null : dbLayer.getObject(tokenCollection,
                                                new GenericFilter({ 'id': pTokenId }));
    },
    async getTokenWithToken(pToken: string): Promise<AuthToken> {
        return IsNullOrEmpty(pToken) ? null : dbLayer.getObject(tokenCollection,
    ...
    async addToken(pAuthToken: AuthToken) : Promise<AuthToken> {
        return dbLayer.createObject(tokenCollection, pAuthToken);
    },
    async removeToken(pAuthToken: AuthToken) : Promise<boolean> {
        return dbLayer.deleteOne(tokenCollection, new GenericFilter({ 'id': pAuthToken.id })); },
    async updateTokenFields(pEntity: AuthToken, pFields: VKeyedCollection): Promise<AuthToken> {
        return dbLayer.updateObjectFields(tokenCollection,
    ...
    async *enumerateAsync(pFilter?: CriteriaFilter, pPager?: CriteriaFilter, pScoper?: CriteriaFilter): AsyncGenerator<AuthToken> {
        for await (const tok of dbLayer.getObjects(tokenCollection, pFilter, pPager, pScoper)) {
        yield tok;
        };
